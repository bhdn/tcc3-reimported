...
para $\sigma$. A figura \ref{fig:loadsimres} apresenta o desempenho desses
parâmetros para utilizando dados da máquina n4. O script utilizado para testes
está no apêndice \ref{chap:programasteste}.

\begin{figure}[htp]
\centering
\includegraphics[width=0.7\textwidth]{src/test-data/checking-load-simulator/load-simulator-n4.png}
\figinfo{Sobreposição do uso de CPU observado na MV e o esperado}
\label{fig:loadsimres}
\end{figure}
...
foram coletados das máquinas observadas, descritas na seção \ref{sec:coleta}. O
intervalo de tempo testado foi de um dia. Neste ambiente simulado foram
adicionados três hospedeiros com 4 CPUs cada. Os testes foram executados na
máquina tcc159. O quadro \ref{quadro:simula0} apresenta os resultados obtidos.

\begin{table}[htp]
\centering
\hspace{-2cm} % FIXME arrumar no template
\quadro{Resultados do teste dos teste de consolidação simulados}\label{quadro:simula0}
