...
treinamento) de uma máquina chamada \emph{generic}. No escopo deste
projeto, fica a cargo do usuário do software escolher qual será o histórico
utilizado para o trainamento de desta máquina especial.
...
que é diferente durante os testes) e então deve-se decidir se cada máquina
virtual deve permanecer no mesmo hospedeiro ou ser migrada para outra com maior
disponibilidade. Para esta decisão é utilizada a estratégia de \emph{first
...
Finalmente, são avaliados diferentes valores em parâmetros utilizados
pelo sistema a ser implementado, a saber:

\begin{itemize}
  \item no caso de $k$-NN, o valor de $k$ é alterado, seguindo as
...
Os resultados para cada combinação de parâmetros são considerados
